                ...
                @foreach($items as $item)
                    <div class="cart-item" id="cart-item-{{ $item['id'] }}">
                        ...
                        <div class="flex-grow-1">
                            ...
                            <div class="text-muted small">S/ {{ number_format($item['price'], 2) }} c/u</div>
                        </div>
                        ...
                        <div class="fw-bold ms-2" id="price-{{ $item['id'] }}" style="min-width:70px; text-align:right;">
                            S/ {{ number_format($item['price'] * $item['quantity'], 2) }}
                        </div>

                        <button class="btn btn-sm text-danger p-1 ms-1" onclick="removeItem({{ $item['id'] }})">
        ...
        async function updateQty(id, delta) {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    // Update item subtotal in UI if you have it
                    const itemSubtotalEl = document.getElementById(`item-subtotal-${id}`);
                    if (itemSubtotalEl && prices[id]) {
                        itemSubtotalEl.textContent = 'S/ ' + (prices[id] * newQty).toFixed(2);
                    }
                } else {
                    Swal.fire({
                        icon: 'warning',
